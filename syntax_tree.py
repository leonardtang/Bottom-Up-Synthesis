...
class ArithmeticSyntaxTree(AbstractSyntaxTree):
    ...
    def combine(self):
        ...
        if self.operator == '+':
            return f'({self.left.combine()} + {self.right.combine()})'
        elif self.operator == 'x':
            return f'({self.left.combine()} x {self.right.combine()})'
        elif self.operator == '/':
            return f'({self.left.combine()} / {self.right.combine()})'
        elif self.operator == '-':
            return f'({self.left.combine()} - {self.right.combine()})'
...
class StringSyntaxTree(AbstractSyntaxTree):
    def __init__(self, operator, left=None, right=None, child=None):
        # TODO: move the common code into AbstractClass
        self.operators = ['concat', 'right', 'left', 'input', 'input_x', 'input_y', 'identity']
        self.unary_operators = ['right', 'left', 'input', 'input_x', 'input_y', 'identity']
        ...
        # self.terminals = [StringSyntaxTree(operator='identity', )]
    ...
    def combine(self):
        ...
        if self.operator in ['input', 'input_x']:
            ...
            return f'Left({self.child.combine()})'
        elif self.operator == 'right':
            return f'Right({self.child.combine()})'
        
        # Binary
        if self.operator == 'concat':
            return f'Concat({self.left.combine()}, {self.right.combine()})'
        else:
            assert False
